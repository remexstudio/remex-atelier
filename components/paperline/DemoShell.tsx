...
import { DemoNav } from "@/components/paperline/DemoNav";
import type { PaperlineDemoHref } from "@/components/paperline/data";

type DemoShellProps = {
  current: PaperlineDemoHref | string;
    ...
    <div className="mx-auto flex min-h-full w-full max-w-3xl flex-col px-6 py-16 sm:px-10 sm:py-24">
      <header className="mb-10 flex items-baseline justify-between gap-6">
        ...
        <nav aria-label="Primary" className="type-meta flex gap-5">
          ...
          <Link href="/work/paperline" className="text-muted no-underline">
            Study
          </Link>
        </nav>
      </header>
      ...
      <footer className="mt-20 border-t border-rule pt-6">
        <p className="type-meta">
          No real payouts. Refunds require Approve. Studio study. Client
          identity anonymized.
        </p>
      </footer>
    </div>
